...
public class ExpressionManager {
            ...
            if (sListeners.containsKey(id)) {
                if (intent.getAction().equals(ACTION_NEW_VALUES)) {
                    ...
                    System.arraycopy(parcelables, 0, timestampedValues, 0,
                            parcelables.length);
                    Log.d("MANAGER SENSOR",timestampedValues.toString());
    ...
    public static List<SensorInfo> getSensors(Context context) {
        ...
        Log.d(TAG, "Found " + discoveredSensors.size() + "interdroid/swancore/sensors");
...
//				Log.d(TAG, "\t" + discoveredSensor.activityInfo.name);
     ...
     *
     * @param context
     * @param name    the entity name of the sensor
     ...
     * @param listener   a {@link TriStateExpressionListener} that receives the
     ...
     *                   contain {@link Expression#SEPARATOR} or end with any of the
     *                   {@link Expression#RESERVED_SUFFIXES}.
     * @param expression the {@link ValueExpression} that should be evaluated
